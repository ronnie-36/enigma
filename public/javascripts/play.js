...
function handleTickInit(tick) {
	...
	var counter = Tick.count.down('2021-03-21T00:00:00+05:30', {
		format: ['h', 'm', 's']
	});

	counter.onupdate = function (value) {
		tick.value = value;
		if (Tick.helper.duration(Tick.helper.date(), Tick.helper.date('2021-03-21T00:00:00+05:30'), ['s'])[0] == 10) {
...
$(document).ready(function () {
	$('.submit_custom').on("click", function (e) {
		...
		if(form.serializeArray()[1].value==""){
			...
			$.post("/play", form.serializeArray(), function (response) {
				if (!response.login) {
					location.reload();
				} else {
					if (response.fun == 0) {
